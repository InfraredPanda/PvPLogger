				...
				if (dmgSource instanceof EntityDamageSource)
				{
					...
					if (entityDamageSrc.getSource() instanceof Player)
					{
						...
						else
						{
							...
							victim.sendMessage(Text.of(TextColors.DARK_RED, "[PvPLogger]: ", TextColors.RED, "You are engaged in combat! Do not logout for ten seconds or you will be punished!"));
							PvPLogger.playersTakenDamage.add(victim.getUniqueId());
						}

						taskBuilder.execute(() -> {
							PvPLogger.playersTakenDamage.remove(victim.getUniqueId());
							victim.sendMessage(Text.of(TextColors.DARK_RED, "[PvPLogger]: ", TextColors.GREEN, "You are no longer in combat, you may disconnect safely."));
